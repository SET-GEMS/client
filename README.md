...
## WebRTC Mesh VS SFU
- Mesh / P2P
  - 송신자와 수신자가 서버 없이 각자의 비디오 및 오디오 등 미디어 스트림을 상대방과 직접 주고 받는 단순한 구조 
...
- SFU(Selective Forwarding Unit)
  - 중앙 서버를 통해 종단간 미디어 트래픽을 중계하는 구조
  - 장점: 참여 인원 수가 늘어나더라도 개인의 하드웨어 및 네트워크 환경에 부담이 적다.
  - 단점: 서버의 부담이 늘어난다.

제가 선택한 것은 WebRTC의 구조는 Mesh 구조였습니다. 가장 큰 이유는 이 프로젝트의 게임 운영 체제를 최대인원 4명의 소규모 방제 시스템으로 선택했기 때문입니다. 한 서버가 여러 방에 있는 모든 유저들의 통신을 모두 관리하는 것보다 각 클라이언트가 각자의 통신을 부담하는 것이 더 비용이 적다고 판단했습니다.

## 주요 기술 스택
- React: 중복되는 코드(컴포넌트, 소켓 이벤트 리스너 등)을 줄이기 위해 선택했습니다. 
- Socket.IO: WebRTC 연결과 실시간 온라인 플레이 구현을 위해 선택했습니다. 
- simple-peer: STUN서버를 기본으로 제공하고, WebRTC 연결 과정을 간소화 시켜주기 때문에 선택했습니다.
- MongoDB & Express: 랭킹 정보 요청과 처리를 간단히 하기 위해 선택했습니다.

## Service Worker의 도입
서비스워커의 도입은 프로젝트를 시작하기 전에도 막연히 해보고싶다고 생각하는 것 중 하나였지만, 이 프로젝트에 서비스워커의 도입을 고려하게 된 계기는 첫 화면의 느린 로딩 속도와 카드의 배경이미지와 보석이미지의 분리때문이었습니다. 두 가지 모두 파일을 불러오는 속도가 느리기 때문에 생기는 현상이기에, 캐시를 저장하여 캐시에서 파일을 불러오는 방식을 지원하는 service worker를 도입하여 로딩 속도를 개선한다면 해결이 될 것이라고 생각했습니다.
